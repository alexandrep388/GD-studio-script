...
Loading.Sidebar:AddLabel("Version : v3.1.0")
task.wait(0.8)
...
local Window = Library:CreateWindow({
    Title = "GD Studio",
    Footer = "Être un Lucky Block • v3.1.0",
    Icon = "zap",
    NotifySide = "Right",
...
MainLeft:AddToggle("AutoRebirth", { Text = "Auto Rebirth", Default = false })
...
MainLeft:AddToggle("AutoClaimGift", { Text = "Auto Claim Playtime Rewards", Default = false })
...
MainLeft:AddToggle("AutoEventPass", { Text = "Auto Claim Event Pass", Default = false })
...
MainLeft:AddToggle("AutoBuyBest", { Text = "Auto Buy Best Luckyblock", Default = false })
    ...
    task.spawn(function()
        while Toggles.AutoBuyBest.Value do
            ...
            if ok and sf then
                ...
                if bestSkin then pcall(function() RFBuySkin:InvokeServer(bestSkin) end) end
            end
            task.wait(0.5)
        end
    end)
...
-- Remove Bad Boss Touch Detectors (garde le chevalier = base15)
local storedParts = {}
FarmRight:AddToggle("RemoveBTD", {
    Text = "Remove Bad Boss Detectors",
    Default = false,
    Tooltip = "Seul le chevalier (base15) peut vous attraper",
})
Toggles.RemoveBTD:OnChanged(function(state)
    ...
    if state then
        ...
        for _, obj in ipairs(folder:GetChildren()) do
            if obj.Name ~= "base15" then -- chevalier
...
-- Teleport to End (base14 = zone collecte)
FarmRight:AddButton("TpToEnd", {
    ...
    Func = function()
        ...
        Library:Notify({ Title = "TP", Content = "Modèles TP à la fin !", Duration = 2 })
    end,
})
...
FarmLeft:AddToggle("AutoFarm", {
    ...
    Default = false,
    Tooltip = "TP le modèle brainrot vers base14",
})
...
Toggles.AutoFarm:OnChanged(function(state)
    ...
    task.spawn(function()
        ...
        while Toggles.AutoFarm.Value do
            ...
            -- ÉTAPE 1 : TP joueur vers le juge
            StatusLabel:SetText("Status : TP vers le juge...")
            root.CFrame = CFrame.new(-528, 48, -2123)
            task.wait(0.3)
            humanoid:MoveTo(Vector3.new(-528, 48, -2123))
            ...
            Library:Notify({ Title = "Auto Farm", Content = "Cycle " .. farmCycles .. " !", Duration = 2 })
...
-- Auto Escape via RUN!! GUI (fiable à 100%)
EscapeLeft:AddToggle("AutoEscape", {
    Text = "Auto Escape",
    Default = false,
    Tooltip = "Détecte RUN!! et TP SafePos automatiquement",
})
...
EscapeLeft:AddButton("TeleportNow", {
    ...
    Func = function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = CFrame.new(715, 39, -2122)
        end
        Library:Notify({ Title = "Escape", Content = "✅ TP effectué !", Duration = 2 })
    end,
})

EscapeRight:AddLabel("SafePos : 715, 39, -2122")
EscapeRight:AddLabel("Juge : -528, 48, -2123")
EscapeRight:AddLabel("Détection via RUN!! GUI")
...
Toggles.AutoEscape:OnChanged(function(state)
    ...
    Library:Notify({ Title = "Auto Escape", Content = "Activé ! Détecte RUN!!", Duration = 2 })

    task.spawn(function()
        -- Attend que le HUD soit disponible
        local gui = player.PlayerGui
        local ok, title = pcall(function()
            return gui:WaitForChild("HUD", 15)
                :WaitForChild("RunTopFrame", 15)
                :WaitForChild("RUN", 15)
                :WaitForChild("Title", 15)
        end)
        if not ok or not title then
            Library:Notify({ Title = "Auto Escape", Content = "Erreur : HUD introuvable", Duration = 3 })
            Toggles.AutoEscape:SetValue(false)
            return
        end

        while Toggles.AutoEscape.Value do
            -- Attend que RUN!! apparaît
            while Toggles.AutoEscape.Value and title.Text ~= "RUN!!" do
                task.wait(0.05)
            end
            if not Toggles.AutoEscape.Value then break end

            -- TP SafePos immédiatement
            local char = player.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                char.HumanoidRootPart.CFrame = CFrame.new(715, 39, -2122)
            end
            ...
            -- Attend que RUN!! disparaît avant de re-détecter
            while Toggles.AutoEscape.Value and title.Text == "RUN!!" do
                task.wait(0.1)
            end
        end
...
Library:Notify({
    Title = "GD Studio",
    Content = "Bienvenue " .. player.Name .. " ! v3.1.0",
    Duration = 3,
})
